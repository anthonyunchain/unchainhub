...
import {
  ClipboardList, ExternalLink, FileText, Image as ImageIcon, Clock, Send, CheckCircle2, Archive,
  Trash2, RefreshCw, CalendarClock, Flame
} from "lucide-react";
...
export default function ClientManagerRequestsTab({ clientId, staffLinked }) {
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select("id, title, description, deadline, importance, files, status, created_at, updated_at")
  ...
  const remove = async (row) => {
    ...
    const { error } = await supabase.from("manager_requests").delete().eq("id", row.id);
    if (error) { toast.error(error.message); return; }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-label-mono" style={{ margin: 0 }}>Manager requests</span>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState
          icon={FileText}
          title="No manager requests yet"
          description="When the manager sends a request from the staff portal, it will show up here."
        />
      ) : (
        <ul className="space-y-3" aria-label="Manager requests">
          {rows.map((s) => {
