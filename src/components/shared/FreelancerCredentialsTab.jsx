...
const CATEGORIES = ["CMS", "Editing", "Design", "Storage", "Client", "Analytics", "Social", "Email", "Other"];
...
export default function FreelancerCredentialsTab({ freelancerId, freelancerName, canEdit = false, tr }) {
  ...
  const load = async () => {
    ...
    setLoading(true);
    const { data, error } = await supabase.rpc("get_freelancer_credentials", { p_freelancer_id: freelancerId });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const { error } = await supabase.rpc("upsert_freelancer_credential", {
      p_id: form.id,
  ...
  const handleDelete = async (r) => {
    const ok = await confirm({
      ...
      description: "This credential will be removed from the freelancer portal.",
      confirmLabel: "Delete",
      destructive: true,
    });
    if (!ok) return;
    const { error } = await supabase.rpc("delete_freelancer_credential", { p_id: r.id });
    if (error) { toast.error(error.message); return; }
  ...
  const logReveal = async (id) => {
    try { await supabase.rpc("log_freelancer_credential_reveal", { p_credential_id: id }); }
    catch { /* best-effort */ }
  };
  ...
  const openLog = async () => {
    ...
    const { data, error } = await supabase
      .from("freelancer_credentials_access_log")
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <span className="text-label-mono" style={{ margin: 0 }}>
            {t("credentials", "Credentials")}{freelancerName ? ` — ${freelancerName}` : ""}
          </span>
        </div>
        {canEdit && (
          <div className="flex items-center gap-2">
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description={rows.length === 0 ? (canEdit
            ? "Add Canva, Frame.io, client CMS, Dropbox, etc."
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3 mt-2">
            <div>
              <Label htmlFor="fcred-label">Label *</Label>
              <Input id="fcred-label" value={form.label}
                onChange={e => setForm(f => ({ ...f, label: e.target.value }))}
                placeholder="Canva, Frame.io, Client CMS, Dropbox…"
                className="mt-1" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label htmlFor="fcred-category">Category</Label>
                <Select value={form.category || "_none"}
                  onValueChange={v => setForm(f => ({ ...f, category: v === "_none" ? "" : v }))}>
                  <SelectTrigger id="fcred-category" className="mt-1"><SelectValue placeholder="Choose…" /></SelectTrigger>
                  <SelectContent>
              ...
              <div>
                <Label htmlFor="fcred-position">Position</Label>
                <Input id="fcred-position" type="number" value={form.position}
                  onChange={e => setForm(f => ({ ...f, position: e.target.value }))}
                  className="mt-1" />
              </div>
            </div>
            <div>
              <Label htmlFor="fcred-url">Login URL</Label>
              <Input id="fcred-url" type="url" value={form.login_url}
                onChange={e => setForm(f => ({ ...f, login_url: e.target.value }))}
                placeholder="https://…"
                className="mt-1" />
            </div>
            <div>
              <Label htmlFor="fcred-username">Username / Email</Label>
              <Input id="fcred-username" value={form.username}
                onChange={e => setForm(f => ({ ...f, username: e.target.value }))}
                autoComplete="off" className="mt-1" />
            </div>
            <div>
              <Label htmlFor="fcred-password">Password</Label>
              <Input id="fcred-password" type="text" value={form.password}
                onChange={e => setForm(f => ({ ...f, password: e.target.value }))}
                autoComplete="off" className="mt-1 font-mono" />
            </div>
            <div>
              <Label htmlFor="fcred-notes">Notes</Label>
              <Textarea id="fcred-notes" rows={2} value={form.notes}
                onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                placeholder="2FA location, scope of access, special instructions…"
                className="mt-1" />
            </div>
      ...
      <Dialog open={logOpen} onOpenChange={setLogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <History className="w-5 h-5 text-brand" /> Access activity{freelancerName ? ` — ${freelancerName}` : ""}
            </DialogTitle>
          </DialogHeader>
          <div className="mt-2">
            {logLoading ? (
