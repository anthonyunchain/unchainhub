  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-label-mono">{tr.photoBankLabel}</p>
      ...
      <div className="grid gap-2" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(min(220px, 100%), 1fr))' }}>
        {photos.map((p, i) => (
          <div key={i} className="relative group rounded-xl overflow-hidden" style={{ aspectRatio: '3 / 4', background: 'var(--card)', border: '1px solid var(--divider)' }}>
            <a href={p.url} target="_blank" rel="noopener noreferrer" className="absolute inset-0 block">
              <img src={photoThumb(p.url)} alt={p.name || `photo ${i + 1}`} loading="lazy" decoding="async" className="absolute inset-0 w-full h-full object-contain transition-transform duration-300 group-hover:scale-105" />
            </a>
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: 'var(--bg)', position: 'relative', zIndex: 1 }}>
      ...
      <div className="mx-auto px-5" style={{ maxWidth: 1400, paddingBottom: 120 }}>
        ...
        {activeTab === 'photos'    && <PhotoBankTab client={client} tr={tr} />}
