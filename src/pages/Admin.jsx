...
function UserManagement() {
  ...
  const ROLE_COLORS = { admin: "bg-violet-50 text-violet-700", user: "bg-blue-50 text-blue-700", freelancer: "bg-amber-50 text-amber-700", client: "bg-teal-50 text-teal-700" };
  ...
  const loadOrphans = async () => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const { data } = await supabase.functions.invoke("auditUsers", {
        headers: { Authorization: `Bearer ${session?.access_token}` },
      });
      setOrphans(data?.orphans || []);
    } catch { setOrphans([]); } finally { setLoadingOrphans(false); }
  };
  ...
  const deleteSelectedOrphans = async () => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      await supabase.functions.invoke("auditUsers", {
        body: { action: "delete", user_ids: selectedOrphans },
        headers: { Authorization: `Bearer ${session?.access_token}` },
      });
      qc.invalidateQueries({ queryKey: ["profiles"] });
      qc.invalidateQueries({ queryKey: ["authUsers"] });
  ...
  return (
    <div>
      <PageHeader title="Users" subtitle="Governance & operations">
        <Button variant="outline" onClick={loadOrphans} disabled={loadingOrphans} className="h-9">
          {loadingOrphans ? "Scanning…" : "Orphaned accounts"}
        </Button>
        ...
        <div className="mb-6 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 bg-slate-50/50">
            <p className="text-sm font-medium text-slate-700">
              {orphans.length === 0 ? "No orphaned accounts found." : `${orphans.length} orphaned account${orphans.length > 1 ? "s" : ""} found`}
            </p>
            <div className="flex gap-2 items-center">
              {selectedOrphans.length > 0 && (
                ...
                >
                  ...
                  {deletingOrphans ? "Deleting…" : `Delete ${selectedOrphans.length} selected`}
                </Button>
              )}
              <button onClick={() => { setOrphans(null); setSelectedOrphans([]); }} className="text-xs text-slate-400 hover:text-slate-600">Dismiss</button>
            </div>
          </div>
          {orphans.length > 0 && (
              ...
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="px-4 py-2 w-8">
                    <input type="checkbox" onChange={e => setSelectedOrphans(e.target.checked ? orphans.map(o => o.id) : [])} checked={selectedOrphans.length === orphans.length} />
                  </th>
                  <th className="text-left text-xs font-medium text-slate-500 px-4 py-2">Email</th>
                  ...
                  <th className="text-left text-xs font-medium text-slate-500 px-4 py-2">Reason</th>
                </tr>
              </thead>
              <tbody>
                {orphans.map(o => (
                  <tr key={o.id} className="border-b border-slate-50 hover:bg-slate-50">
                    <td className="px-4 py-2">
                      <input type="checkbox" checked={selectedOrphans.includes(o.id)} onChange={e => setSelectedOrphans(e.target.checked ? [...selectedOrphans, o.id] : selectedOrphans.filter(id => id !== o.id))} />
                    </td>
                    <td className="px-4 py-2 text-sm text-slate-700">{o.email || "—"}</td>
                    <td className="px-4 py-2 text-sm text-slate-500">{o.full_name || "—"}</td>
                    <td className="px-4 py-2 text-xs"><span className={`px-2 py-0.5 rounded-full font-medium ${ROLE_COLORS[o.role] || "bg-slate-100 text-slate-600"}`}>{o.role || "none"}</span></td>
                    <td className="px-4 py-2 text-xs text-amber-600">{o.reason}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div key={sec.label}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {sec.items.map(item => {
                  ...
                  return (
                    <button key={item.id} onClick={() => setSection(item.id)} className={cls}>
                      {item.label}
                      {badges[item.id] && (
