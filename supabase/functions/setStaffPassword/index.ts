...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!client_id) return Response.json({ error: 'client_id is required' }, { status: 400, headers: corsHeaders(req) });
    ...
    if (existingUserId) {
      ...
      if (existingRole === 'admin') {
        return Response.json({ error: 'This email belongs to an admin account. Cannot overwrite.' }, { status: 400, headers: corsHeaders(req) });
      }

      // Allow seamless overwrite for existing staff; require force for other non-staff roles
      if (existingRole && existingRole !== 'staff' && !force) {
      ...
      const { data: createData, error: createErr } = await supabaseAdmin.auth.admin.createUser({
        ...
        password,
        email_confirm: true,
        user_metadata: { company_name, role: 'staff' },
      });
    ...
    // Link this user as the staff of the given client
    await supabaseAdmin.from('clients').update({ staff_user_id: userId }).eq('id', client_id);
