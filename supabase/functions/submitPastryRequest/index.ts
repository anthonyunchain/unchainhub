...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { title, description, deadline, files } = await req.json();
    ...
    try {
      const notifTitle = `🥐 Pastry/Baker request: ${client.company_name}`;
      const deadlineTag = deadline ? ` · Due ${deadline}` : '';
      const notifMessage = `${title.trim()}${deadlineTag}`;
      ...
      console.error('[submitPastryRequest] notif non-fatal:', (notifErr as Error)?.message);
    }
