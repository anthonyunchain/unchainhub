...
const IMPORTANCE = new Set(['low', 'medium', 'high', 'urgent']);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { title, description, deadline, importance, files } = await req.json();
    if (!title || typeof title !== 'string' || !title.trim()) {
      return Response.json({ error: 'Title is required' }, { status: 400, headers: corsHeaders(req) });
    }

    const importanceClean = IMPORTANCE.has(importance) ? importance : 'medium';
    ...
    try {
      const notifTitle = `📋 Manager request: ${client.company_name}`;
      const importanceTag = importanceClean === 'urgent' ? ' · URGENT' : importanceClean === 'high' ? ' · High' : '';
      const deadlineTag = deadline ? ` · Due ${deadline}` : '';
      const notifMessage = `${title.trim()}${importanceTag}${deadlineTag}`;
      ...
      console.error('[submitManagerRequest] notif non-fatal:', (notifErr as Error)?.message);
    }
